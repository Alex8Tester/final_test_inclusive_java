...
public class DataParse {
    public HashMap<String, Object> parseInputData() {
        ...
        HashMap<String, Object> dataDiction = new HashMap<>();
        StringBuilder sb = new StringBuilder();
        for (String i : data) {
            if (i.length() == 1) {
                if (i.equals("f") || i.equals("m")) {
                    dataDiction.put("sex", i);
                } else{
                    try {
        ...
        if (fullName.length == 3) {
            dataDiction.put("lastName", fullName[0]);
            dataDiction.put("firstName", fullName[1]);
            dataDiction.put("patronymic", fullName[2]);
        }
        return dataDiction;
    }
}
